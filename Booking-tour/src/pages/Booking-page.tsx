...
const BookingPage = () => {
  ...
  const statusBooking: OptionType[] = [
    { label: "Chưa bắt đầu", value: "NotYet" },
    { label: "Đã bắt đầu", value: "Going" },
    { label: "Đã kết thúc", value: "Finish" },
      ...
      >
        ...
        <Row className="search__filter">
          <Col span={6}>
            ...
            <Form.Item name="typeRest" label="Trạng thái đơn đặt">
              <Checkbox.Group>
                <Row gutter={[0, 12]}>
                  {statusBooking.map((data, key) => (
                    <Col span={24} key={key}>
                      <Checkbox value={data.value}>{data.label}</Checkbox>
                    </Col>
                  ))}
